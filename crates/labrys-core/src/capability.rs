...
//! operations.
//!
//! This module is pure and deterministic. [`SecretStore`] models the sealed
//! envelope contract (master key held outside the store, mirroring encrypted
//! PostgreSQL with the key outside the database); the actual cipher belongs to
...
impl SecretStore {
    ...
    pub fn put(
        ...
        let ciphertext = seal(&self.master_key, &ref_id, value);
    ...
    /// store only here; callers must not log the result.
    pub fn inject(&self, grant: &InjectionGrant, ref_id: &str) -> Result<String> {
        ...
        open(&self.master_key, sealed)
            .ok_or_else(|| CoreError::SecretNotFound(format!("secret '{ref_id}' failed to open")))
    }
    ...
    /// Re-seals an existing value under a new version (rotation). The value
    /// itself is unchanged; consumers must pick up the new reference.
    pub fn rotate(
        &mut self,
        ref_id: &str,
        actor: impl Into<String>,
        ...
        let sealed = self
            ...
            .get(ref_id)
            .ok_or_else(|| CoreError::SecretNotFound(format!("secret '{ref_id}' not found")))?;
        let value = open(&self.master_key, sealed).ok_or_else(|| {
            CoreError::SecretNotFound(format!("secret '{ref_id}' failed to open"))
        })?;
        ...
        self.secrets.insert(
            ...
            SealedSecret {
                ...
                version,
                scope,
                ciphertext: seal(&self.master_key, ref_id, &value),
            },
        );
        self.audit.push(SecretAudit {
            ref_id: ref_id.to_string(),
...
/// Deterministic keyed seal (XOR stream derived from the master key and the
/// reference id). Stands in for the MVP encrypted-PostgreSQL cipher; the
/// contract — ciphertext at rest, plaintext only under the external key — is
/// what this module tests.
fn seal(master_key: &MasterKey, ref_id: &str, plaintext: &str) -> Vec<u8> {
